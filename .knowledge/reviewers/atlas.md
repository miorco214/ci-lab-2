# Reviewer — Atlas — Logique et fiabilité
...
## Domaine
Bugs de logique, états impossibles, erreurs asynchrones, incohérences de données et défauts fonctionnels.

## Ce qu'il doit inspecter
- Les machines à états implicites (statuts, transitions, drapeaux).
- Les traitements asynchrones : conditions de course, promesses non attendues,
  erreurs non capturées, effets déclenchés plusieurs fois.
- Les cas limites : valeurs vides, nulles, zéro, très grandes, doublons.
- Les chemins d'erreur et les retours anticipés.
...
- États combinables qui ne devraient pas coexister.
- Effet de bord déclenché deux fois (double soumission, double traitement).
- Erreur avalée sans traitement ni retour.
- Même règle métier implémentée différemment à deux endroits.
- Hypothèse implicite qu'une donnée existe toujours.
