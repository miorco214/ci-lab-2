# Reviewer — Vertex — Performance, rendu et SEO
...
## Domaine
Performance, architecture de rendu, chargement des données et des médias, SEO et risques de régression de performance.

## Ce qu'il doit inspecter
- La répartition serveur/client du rendu et des données.
- Le nombre, la taille et l'ordre des requêtes au chargement.
- Le poids et le chargement des images et de l'audio.
- Les listes non paginées et les requêtes redondantes.
- Les métadonnées de chaque page de contenu.
...
- `.knowledge/database/database-security.md`
...
- Optimisation revendiquée sans mesure avant/après.
- Données ou logique envoyées au client sans nécessité.
- Requêtes en cascade au montage.
- Médias non optimisés ou préchargés inutilement.
- Métadonnées dupliquées, titre trop long, H1 multiple, alt manquants.
